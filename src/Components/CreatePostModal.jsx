...
const CreatePostModal = (props) => {
  ...
  return (
    ...
    >
      <p className="text-white mb-5 text-center">New thread</p>
      {/* modal */}
      <div className="dark:bg-zinc-800 bg-white min-h-[100px] rounded-2xl border-[1px] border-zinc-700 p-6">
        {/* header */}
        <div className="flex items-center">
          {/* userimage */}
          <div className="dark:bg-zinc-600 bg-zinc-200 min-h-[50px] min-w-[50px] rounded-full mr-3"></div>

          <div className="w-full">
            <p className="dark:text-white text-zinc-800">Dustin U</p>
            <input
              onChange={(e) => {
                setPostVal(e.target.value);
              }}
              value={postVal}
              className="bg-transparent outline-none font-light text-zinc-600 dark:text-white text-lg w-full"
              type="text"
        ...
        {/* options & media preview */}
        <div className="my-5 py-2 border-l-[2px] border-zinc-300 dark:border-zinc-600 pl-5 ml-[25px]">
          {currentGif !== "" && (
            <SelectedGif
          ...
          <ul className=" flex items-center gap-4 text-zinc-500 dark:text-[#767D83] text-[20px] cursor-pointer">
            {/* heart */}
            <li>
        ...
        <div className="ml-3  flex items-center  disabled">
          <div className="bg-zinc-400 dark:bg-zinc-600 min-h-[30px] min-w-[30px] rounded-full mr-3"></div>
          <input
        ...
        <div className="flex justify-between items-center mt-10">
          ...
          <PostBtn
            onClick={() => {
              const postText = postVal;
              const postGif = currentGif !== "" ? currentGif : null;

              setPosts((prevPosts) => [
                ...prevPosts,
                { text: postText, gif: postGif },
              ]);
              setCreatePost(false);
              setOverlay(false);
              setCurrentGif("");
